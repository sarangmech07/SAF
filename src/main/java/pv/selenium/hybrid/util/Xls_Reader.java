...
public class Xls_Reader {
	public static String filename = System.getProperty(Constants.USER_DIR) + "\\src\\config\\testcases\\TestData.xlsx";
	public String path;
	...
	public String getCellDataRandom(String sheetName, String colName, int rowNum) {
		try {
			// fis = new FileInputStream(path);
			...
			for (int i = 0; i < row.getLastCellNum(); i++) {
				// System.out.println(row.getCell(i).getStringCellValue().trim());
				if (row.getCell(i).getStringCellValue().trim().equals(colName.trim())) {
					// System.out.println("Column Name"+row.getCell(i).getStringCellValue().trim());
					colNum = i;
					break;
				}
			}
